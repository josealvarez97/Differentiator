...
string CExpression::toPrefix(string exp)
{
	...
	for (int i = exp.length(); i > 0; i--)
	{
		...
		if (isOperator(currentItem) || isUnaryOperator(currentItem))
		{
			...
			while(!operators.empty() 
				...
				&& operators.top() != ")" // EXCEPT WHEN THE TOP ELEMENT IN THE STACK IS A CLOSING PARENTHESIS
				&& currentItem != ")"
				&& currentItem != "(") 
			{
...
{
	const string digits = "0123456789ABDEFGHIJKLMNOPQRUVWXYZ";
